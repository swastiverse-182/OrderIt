...
exports.adminSignup = catchAsyncErrors(async (req, res, next) => {
  ...
  // Create user and FORCE the role to admin
  // NOTE: Ensure this route is protected so only existing admins can access it
  const user = await User.create({
    name,
    email,
    password,
...
exports.protect = catchAsyncErrors(async (req, res, next) => {
  ...
  // FIX: Check that cookie is not the "loggedout" placeholder value
...
exports.updatePassword = catchAsyncErrors(async (req, res, next) => {
  ...
  // Send a new token after password change
  sendToken(user, 200, res);
...
exports.updateProfile = catchAsyncErrors(async (req, res, next) => {
  ...
  if (req.body.avatar !== "") {
    ...
    // Skip Cloudinary destroy if user has the default avatar
    if (image_id !== "default") {
...
exports.logout = catchAsyncErrors(async (req, res, next) => {
  ...
  // FIX: Added sameSite: "lax" for consistent cookie behavior
  res.cookie("jwt", "loggedout", {
    expires: new Date(Date.now() + 10 * 1000), // expires in 10 seconds
    httpOnly: true,
    sameSite: "lax", // FIX: Required for cross-port cookie handling
  });
